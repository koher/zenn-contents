...
**Swift 未経験者が Swift で競技プログラミングに挑戦してみるための**、 Swift の基本構文や標準ライブラリの**チートシートです**。競技プログラミングで必要そうなものに絞って掲載しています。

ページ右下の "Contents" から各項目に素早くアクセスできるのでご活用下さい。
...
`print` の後は改行します。改行をなくすには次のように書きます。
...
定数宣言と初期化は別々にすることができ、条件分岐と組合せて初期化することもできます。
...
インクリメントおよびデクリメントのための演算子はないので、 `+=` や `-=` を利用します。
...
Swift の演算子にはオーバーフローが許されていません。オーバーフローした場合は実行時エラーになります。意図的にオーバーフローさせたい場合には `&` を付けて `&+`, `&-` などと書きます。
...
Swift の `Int` は実行環境によって 32 bit か 64 bit か変わりますが、 **2020 年現在で Swift が動作する環境では基本的に 64 bit** と考えて良いです。

今の実行環境でどちらかを調べるには次のようなコードをします。
...
**Swift の整数型はオーバーフローすると実行時エラー**になるので、オーバーフローを利用した演算が必要な場合は、オーバーフローの項を参考に適切に `&+` や `&-` 等を利用して下さい。
...
`String` からの変換に失敗した場合（ `"abc"` のような整数に変換できない文字列が与えられた場合など）、 `nil` が返されます。そのため戻り値の型は `Int` ではなく `Int?` （ `Optional<Int>` ）になります。競プロでは失敗するような変換を行わないことが多いので、ほとんどのケースは `!` を付けて `nil` を無視し、 `Int` に変換することができます。
...
わかりやすいように `_` をリテラル中に挿入することもできます。
...
```swift
// この場合は 42 というリテラルは UInt 型
let a: UInt = 42
```

これは `42` が `Int` で `UInt` に暗黙の型変換されているのではなく、 `UInt` 型の `a` に代入されることから `42` というリテラルの型が `UInt` に決定されています。

型の記述が省略された場合は、整数リテラルは `Int` 型と推論されます。

```swift
// この場合は 42 というリテラルは Int 型
let a = 42
```
...
競プロにおいては、結果として複数値を出力しなければならない場合などに便利です。
...
Swift では部分文字列型 `Substring` は `String` と型として区別されます。 `String` に戻す必要がある場合は↓のようにして変換します。
...
↑で `!` が必要なのは `nil` が返される可能性があるからです。空文字列の場合 `first` で最初の文字を取得することができないため `nil` が返されます。 `nil` が返される可能性があるため、 `first` の戻り値の型は `Character?` （ `Optional<Character>` ）です。空文字列でないことがわかっている場合、 `!` で `nil` を無視し、 `Character?` を `Character` に変換できます。
...
`[Int]` は `Array<Int>` のシンタックスシュガーです。どちらでも書けますが簡潔な `[Int]` が用いられることが多いです。↓の二つはまったく同じ意味です。
...
代入するだけでコピーされるのがパフォーマンスに悪影響を与えそうですが、実際には [Copy-on-Write](https://ja.wikipedia.org/wiki/%E3%82%B3%E3%83%94%E3%83%BC%E3%82%AA%E3%83%B3%E3%83%A9%E3%82%A4%E3%83%88) という仕組みを使い、代入時にはバッファを共有して変更時に必要になるまでコピーを行いません。
...
`removeLast` および `remove` は空の `Array` に対して呼び出すと実行時エラーになりますが、 `popLast` の場合は `nil` を返します。
...
これを利用して、 `popLast` を用いて要素がなくなるまでループすることができます。
...
Swift では部分配列型 `ArraySlice` は `Array` と型として区別されます。 `Array` に戻す必要がある場合は↓のようにして変換します。

```swift
let a: [Int] = [0, 1, 1, 2, 3, 5]
let b: ArraySlice<Int> = a[1 ..< 3] // [1, 1, 2] ( ArraySlice<Int> )
let c: [Int] = Array(a)             // [1, 1, 2] ( [Int] )
```
...
`Array` と違って `Dictionary` をキーで引く場合、キーに対応した値が登録されていないと `nil` が返されます。そのため、 `a["x"]` の戻り値の型は `Int` ではなく `Int?` （ `Optional<Int>` ）になります。キーに対応した値が存在していることがわかっている場合は `a["x"]!` のように `!` を付けることで `nil` を無視し、（このケースであれば） `Int?` を `Int` に変換することができます。
...
`Dictionary` も `Array` と同じく `struct` で値型です。変更するためには `let` （定数）ではなく `var` （変数）に格納されていなければいけません。
...
**Swift のタプルは値型であり、タプル型変数はスタック領域に確保されるので（オブジェクトに包まれるなどの）オーバーヘッドがありません。**
...
↑の例で `a` を `Int` として使うためには `a` が `nil` でないことを確認する必要があります。
...
この `if let a = a` のような代入を Optional Binding といい、新しい `a` の型は `Int` になります。
...
`if` の後の条件式に `()` は不要です。 `{}` は必須です。ただし、条件式に Trailing Closure を書く場合はパースの問題で　`()` が必要です。

```swift
// 条件式に () が必要な場合
if (array.reduce(0) { $0 + $1 * $1 }) {
    // ...
}
```
...
制約のおかげで `T.zero` や `sum += element` における `T` 同士のたし算が実現されています。
...
`map` 等と組み合わせると次のようなことができます。入力が↓の場合、

```
2 3 5
```

↓のようにして `[2, 3, 5]` を得ることができます。
